...
def fgsm_targeted(x, grad, eps):
    # project once in the right direction
    adv = x - eps * grad.sign()
    # make sure the perturbed image is in [0,1]
    return torch.clamp(adv, 0.0, 1.0)

# sweep eps to find the smallest that achieves target
best = None
for epsReal in [i/100 for i in range(1,101)]:  
    adv_x = fgsm_targeted(x, x.grad, epsReal - 1e-7)
    new_class = N(adv_x).argmax(1).item()
    if new_class == t:
        best = epsReal
        break

print("Original class:", N(x).argmax(1).item())
print("Best single-step epsReal that hits target:", best)
